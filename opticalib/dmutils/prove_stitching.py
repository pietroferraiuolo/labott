...
@timer
def map_stitching_fast(
    zz: np.ndarray, fullmask: np.ndarray, idr: List[int]
) -> np.ndarray:
    """
...
def _assemble_system_memory_efficient(
    zz: np.ndarray, zernike_basis: np.ndarray
...
def _compute_q_block_efficient(
    zernike_basis: np.ndarray, overlap: np.ndarray
) -> np.ndarray:
    """
...
def _compute_p_block_efficient(
    zernike_basis: np.ndarray, img_diff: np.ndarray, overlap: np.ndarray
) -> np.ndarray:
    """
...
def _apply_corrections_memory_efficient(
    zz: np.ndarray, fullmask: np.ndarray, idr: List[int], coefficients: np.ndarray
) -> np.ndarray:
    """
    ...
    final_result.data[valid_average] = (
        result_sum.data[valid_average] / result_count[valid_average]
    )
...
@timer
def map_stitching_chunked(
    zz: np.ndarray, fullmask: np.ndarray, idr: List[int], chunk_size: int = 5
) -> np.ndarray:
    """
    ...
    final_result.data[valid_avg] = (
        result_accumulator.data[valid_avg] / weight_accumulator[valid_avg]
    )
